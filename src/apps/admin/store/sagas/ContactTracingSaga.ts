...
function* getContactTracingData(payload: ContactTracingRequest): IterableIterator<any> {

  try {
    const response: undefined | ContactTracingResponse = yield ContactTracingService.contactTrace(payload);
...
function* authWatcher(): IterableIterator<any> {
  yield takeLatest(ContactTracingActionTypes.CONTACT_TRACING_REQUEST, (action: ContactTracingAction) => getContactTracingData(action.data as ContactTracingRequest));
}
